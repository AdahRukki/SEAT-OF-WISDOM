...
-   **Payment Tracking & Reconciliation System**: Comprehensive fee payment management with:
    ...
    -   **Offline-First Recording**: Bursars can record payments offline with automatic sync when online. On every Payment Recording mount, an interrupted-submission reconciler scans the persisted optimistic rows (`pendingPayments` in localStorage) and, for any row in a transient state ('saving' / 'pending-sync' / 'pending-slow') whose `clientRequestId` is NOT in the current offline queue, replays the original `__submission` once via `queuedApiRequest`. The `clientRequestId` unique index on `fee_payment_records` guarantees the server dedupes if the original actually completed; otherwise the row is (re)created or (re)queued. Failures land in the existing Failed/Retry UI. Reconciliation runs only against rows hydrated from localStorage at mount (snapshot via `hydratedPendingRef`) so a brand-new in-flight optimistic row is never misclassified as an orphan.
    -   **Finance Tab UX**: Term/session selectors at top of Finance tab to switch contexts. Summary cards (Total Revenue, Outstanding Fees, Collection Rate, Students Owing) now compute from `fee_payment_records` (confirmed payments) rather than legacy `student_fees` table only. Payment records table has date-range filter (From/To date inputs) and pagination (25 rows/page with Previous/Next navigation). `getFinancialSummary` returns `totalRevenue`, `actualTuitionCollected`, `totalOutstanding`, `collectionRate`, `studentsOwing` in addition to legacy fields. `actualTuitionCollected` = sum across active students of `min(theirConfirmedTuitionPayments, theirAssignedTuition)` — used for `totalOutstanding` and `collectionRate` so overpayments don't inflate them. `studentsOwing` counts active students whose confirmed tuition payments are LESS than their assigned tuition (partial payers included). `totalRevenue` remains the raw sum of all confirmed payment records (any purpose). Total Revenue card shows an "Actual tuition" sub-line.
