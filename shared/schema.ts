...
// Bulk report generation requests table
export const bulkReportRequests = pgTable("bulk_report_requests", {
  id: uuid("id").primaryKey().default(sql`gen_random_uuid()`),
  schoolId: uuid("school_id").references(() => schools.id, { onDelete: "cascade" }),
  classId: varchar("class_id", { length: 50 }).references(() => classes.id),
  term: varchar("term", { length: 20 }).notNull(),
  session: varchar("session", { length: 20 }).notNull(),
  requestType: varchar("request_type", { length: 20 }).notNull(), // 'school' or 'class'
  status: varchar("status", { length: 20 }).default("pending"), // 'pending', 'processing', 'completed', 'failed'
  totalStudents: integer("total_students").default(0),
  processedStudents: integer("processed_students").default(0),
...
// Bulk report generation schemas
export const bulkReportRequestSchema = createInsertSchema(bulkReportRequests);
export const createBulkReportRequestSchema = bulkReportRequestSchema.omit({
  id: true,
  createdAt: true,
  updatedAt: true,
  completedAt: true,
...
// Term settings update schema
export const updateTermSettingsSchema = z.object({
  termId: z.string().uuid(),
  resumptionDate: z.string().nullable(),
  currentTerm: z.string().optional(),
  currentSession: z.string().optional(),
});

// Types for bulk report generation
export type BulkReportRequest = typeof bulkReportRequests.$inferSelect;
export type InsertBulkReportRequest = z.infer<typeof insertBulkReportRequestSchema>;
export type CreateBulkReportRequest = z.infer<typeof createBulkReportRequestSchema>;
export type UpdateTermSettings = z.infer<typeof updateTermSettingsSchema>;

// Bulk report generation request type
export const insertBulkReportRequestSchema = createInsertSchema(bulkReportRequests).omit({
  id: true,
  createdAt: true,
  updatedAt: true,
});

// Progress response for bulk generation
export type BulkReportProgress = {
  id: string;
